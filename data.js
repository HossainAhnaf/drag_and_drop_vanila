class UseHorizontalDragableList {
  ...
  #updateState(newElm, newDraggingElementVisualIndex) {
    ...
    function updateNextPrevState() {
      const visualIndexArr = this.draggableElements.map(elm => parseInt(elm.getAttribute("data-visual-index")))
      console.log(visualIndexArr);
      const index = visualIndexArr.indexOf(newDraggingElementVisualIndex)

      this.prevState.elm = this.draggableElements[index - 1] || null
      this.nextState.elm = this.draggableElements[index + 1] || null

       this.prevState.rect = this.prevState.elm?.getBoundingClientRect()
       this.nextState.rect = this.nextState.elm?.getBoundingClientRect()
    }
  ...
  #dragMoveHandler(e) {
    if (this.draggingState.elm?.getAttribute("data-dragging") == "true") {
      // console.clear()
      // console.log(this.prevState.elm?.textContent, this.nextState.elm?.textContent);
